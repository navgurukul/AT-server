...
export class TimesheetsService {
  ...
  async getMonthlyDashboard(params: {
    ...
    // Calculate payable days: week-off days + working days with timesheets + working days with approved paid leaves
    let weekOffDays = 0;
    let timesheetDays = 0;
    let paidLeaveDays = 0;

    for (const day of days) {
      // Count week-off days (weekends + holidays)
      if (!day.isWorkingDay) {
        weekOffDays++;
        continue;
      }

      // Count working days with timesheets
      if (day.timesheet) {
        timesheetDays++;
        continue;
      }

      // Count working days with approved paid leaves
      if (day.leaves && day.leaves.entries.some(e => e.state === 'approved')) {
        paidLeaveDays++;
      }
    }

    const totalPayableDays = weekOffDays + timesheetDays + paidLeaveDays;
